...
public class MongoConfigR {
    ...
    @Bean
    public MongoDBSecretR dbSecret(@Value("${spring.data.mongodb.uri}") String uri) {
        return MongoDBSecretR.builder()
                .uri(uri)
                .build();
    }

    @Bean
    public ReactiveMongoClientFactory mongoProperties(MongoDBSecretR secret) {
